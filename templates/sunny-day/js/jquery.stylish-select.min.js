...
(function($){$('html').addClass('stylish-select');$.fn.extend({getSetSSValue:function(value){if(value){$(this).val(value).change();return this;}else{return $(this).find(':selected').val();}},resetSS:function(){var oldOpts=$(this).data('ssOpts');$this=$(this);$this.next().remove();$this.unbind('.sSelect').sSelect(oldOpts);}});$.fn.sSelect=function(options){return this.each(function(){var defaults={defaultText:'Please select',animationSpeed:0,ddMaxHeight:'',containerClass:''};var opts=$.extend(defaults,options),$input=$(this),$containerDivText=$('<div class="selectedTxt"></div>'),$containerDiv=$('<div class="newListSelected '+opts.containerClass+'"></div>'),$newUl=$('<ul class="newList" style="visibility:hidden;"></ul>'),itemIndex=-1,currentIndex=-1,keys=[],prevKey=false,prevented=false,$newLi;$(this).data('ssOpts',options);$containerDiv.insertAfter($input);$containerDiv.attr("tabindex",$input.attr("tabindex")||"0");$containerDivText.prependTo($containerDiv);$newUl.appendTo($containerDiv);$input.hide();$containerDivText.data('ssReRender',!$containerDivText.is(':visible'));if($input.children('optgroup').length==0){$input.children().each(function(i){var option=$(this).html();var key=$(this).val();keys.push(option.charAt(0).toLowerCase());if($(this).attr('selected')==true){opts.defaultText=option;currentIndex=i;}
...
var text=$newLi.eq(currentIndex).html();var val=$newLi.eq(currentIndex).parent().data('key');if(init==true){$input.val(val);$containerDivText.html(text).val();return false;}
try{$input.val(val)}catch(ex){$input[0].selectedIndex=currentIndex;}
$input.change();$containerDivText.html(text).val();}
...
keyPressed=String.fromCharCode(keycode).toLowerCase();for(var i=(start||0);i<keys.length;i++){if(keys[i]==keyPressed){var currentKeyIndex=i;}}
if(typeof currentKeyIndex!='undefined'){++currentIndex;for(var i=currentIndex;i<keys.length;i++){if(keys[i]==keyPressed){currentIndex=i;}}
if(currentIndex==-1||currentIndex==null||prevKey!=keyPressed){for(var i=currentIndex;i<keys.length;i++){if(keys[i]==keyPressed){currentIndex=i;}}}
