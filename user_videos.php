...
if ( trim($user_data['name']) == '' && trim($user_data['title']) == '' ) {
	if ( $_GET['p'] == 1 ) {
		$ref = '/profile/'.$profile_id.'/videos/';
	} else {
		$ref = '/profile/'.$profile_id.'/videos/page'.$_GET['p'].'.html';
	}
...
$data_videos   	= YT_GetUserUpload($_GET['u'], $_GET['p'], $config['user_uploaded_list_per_page']);
...
if((($_GET['p']) * $config['list_per_page']) <= $total_user_upload_videos) {
  $next_page = $_GET['p'] + 1;
}
if($_GET['p']>1) {
  $prev_page = $_GET['p'] - 1;
