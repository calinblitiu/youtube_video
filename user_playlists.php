...
$data_videos	= YT_GetUserPlaylists($_GET['u'], $_GET['p'], $config['list_per_page']);

$total_videos			= YT_Total_Videos($data_videos);
$user_playlist_videos	= YT_Videos($data_videos);
$temp_videos			= array();

$idx = 0;
foreach($user_playlist_videos as $user_playlist_video) {
	if ( $user_playlist_video['size'] > 0 ) {
		$temp_videos[$idx] = $user_playlist_video;
		$idx++;
	}
}
$user_playlist_videos	= $temp_videos;
...
$tpl->assign('upload_count', $total_videos);
$tpl->assign('user_playlists', $user_playlist_videos);
...
$tpl->display('user_playlists.html');
