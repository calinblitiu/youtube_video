...
if ( trim($user_data['name']) == '' && trim($user_data['title']) == '' ) {
	...
	if ( $_GET['p'] == 1 ) {
		$ref = '/profile/'.$profile_id.'/favorites/';
	} else {
		$ref = '/profile/'.$profile_id.'/favorites/page'.$_GET['p'].'.html';
	}
...
$data_videos	 		= YT_GetUserFavorites($_GET['u'], $_GET['p'], $config['list_per_page']);
...
if((($_GET['p']) * $config['list_per_page']) < $total_user_fav_videos) {
  $next_page = $_GET['p'] + 1;
}
if($_GET['p']>1) {
  $prev_page = $_GET['p'] - 1;
...
$tpl->assign('upload_count', $total_user_fav_videos);
$tpl->assign('user_favorites', $user_fav_videos);
$tpl->assign('total', ceil($total_user_fav_videos / $config['list_per_page']));
...
$tpl->display('user_favorites.html');
